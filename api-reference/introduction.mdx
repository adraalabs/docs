...
Write endpoints are rate limited — 60 requests per minute for conversation writes (assign, send message) and 30 per minute for agent and contact writes; each endpoint's reference page lists its limit. All other endpoints accept 100 requests per minute per IP. Exceeding a limit returns `429 RATE_LIMIT_EXCEEDED`.
